...
//	document.getElementById('tid').value = '';
	...
	if(sid){
		//변수에 데이터가 채워져 있는 경우(null 또는 undefined가 )
		document.getElementById('tpw').focus();
	} else{
		//sid에 데이터가 채워져 있지 않은 경우(null 또는 undefined)
...
// 유효성 검사를 하고
// 검사에 통과하면 text.html 로 아이디로 비밀번호를 전달하세요.
...
//		location.href = 'test.html?id=' + sid + '&pw=' + sqw; // GET 방식 전송

		// 데이터전송용 form 태그의 input 태그에 읽은 데이터를 입력해서 전송하세요.
		// 1. 아이디가 id, pw 인 태그에 전송데이터를 채워 넣는다.
